...
- Passed `--no-config`: goojust's installer would otherwise seed a starter
  `config.toml`, and we ship our own below. `run-component.sh` copies `files/`
  after `component.sh`, so our overlay would win regardless, but skipping the
...
- `usr/share/goojust/justfile` — the system justfile, referenced by the
  config above. Imports `justfile.apps`, which `run-component.sh` builds at
