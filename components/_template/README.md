[root](../../../README.md) / [build-phases](../../build-phases/README.md) / [components](../README.md) / **_template**
...
1. Copy the inner component directory to its destination. The path relative
   to `components/` is the entry you'll add to components.list:
   ```
   cp -r components/_template/component-name components/<group-or-name>/<name>
   ```
...
4. Add the component path (e.g. `core/my-component`) to `components.list` in
...
- A component under a `[desktop]` / `[laptop]` section is gated to that flavor
  ...
  `Containerfile.base` `ARG FLAVORS` build arg.
...
- **shellcheck runs in CI** on every `*.sh` and on `files/{libexec,system-generators}`
  ...
  disable=SC2034`.
- **The components.list entry IS the path** relative to `components/`. E.g.
  `core/my-component` for `components/core/my-component/`. No uniqueness
