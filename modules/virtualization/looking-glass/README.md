...
Builds the Looking Glass `kvmfr` DKMS module, the shared-memory transport between the host and a GPU-passthrough VM. Desktop flavor only (gated by the `flavor "desktop"` block in [modules.kdl](../../../modules.kdl)); the matching VFIO kargs, modprobe config and rebind service ship in the `vfio-passthrough` module.
