# shellcheck disable=SC2034  # versions here are consumed by module.sh
# Renovate-tracked pins for the fetched cli-customizations binaries. Keep
# each annotation comment directly above its version line.
...
# HalFrgrd/flyline publishes a sha256 sidecar per release asset, so this pin
# starts accurate rather than trust-on-first-use; checksums.yml keeps it in
# sync with FLYLINE_VERSION bumps.
