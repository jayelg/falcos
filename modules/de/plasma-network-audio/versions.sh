...
# Renovate-tracked pin for the plasma-network-audio RPM.
# The RPM asset filename doesn't derive from the tag, so a Renovate bump
# here needs a manual follow-up: the filename in module.sh and the
# SHA256 below (out of checksums.yml's reach for the same reason).
