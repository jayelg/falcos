...
use crate::list::{Entry, Image, List, NO_FLAVOR};
...
/// in scope is part of the cache key of every RUN under it. No module
/// layer needs any of it, because no module spells the brand out.
///
/// An undeclared field is left out rather than emitted empty, so the
/// branding helper's own default applies instead of an empty string
...
/// image version, the image's own identity and the finalize hook order,
/// plus lib, the module directories and the brand assets. One above them
/// sees none of that and mounts only its own script: those ARGs are not
/// declared yet, and binding the module tree into the first layer of the
/// build would put every module's content in its cache key.
...
/// Every verify diagnostic the enabled modules accept, unique, one per
/// line, pipe separated:
///
///     <class>|<unit>
///
/// Resolved per target for the same reason `contract-files` is: an
/// exception belongs to the module that ships the unit, so an image
/// without that module must not carry it. That is what makes delisting a
/// module take its exceptions with it.
///
/// The class name travels, never the pattern it stands for. The patterns
