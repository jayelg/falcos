//! Typed options: declared by the module, set by the image author,
//! resolved here and passed to the layer as env.
//!
//! What this turns into a surface is the hardcoded list inside a module's
//! own script, which an image author could previously only change by
...
pub fn parse_option(node: &KdlNode, file: &str, text: &str, issues: &mut Issues) -> Option<Opt> {
    ...
    // A list entry's `source` child is the out-of-tree pin, so an option
    // under that name could be declared but never set.
    if name == "source" {
        issues.push(
    ...
    for child in node.children().map(|c| c.nodes()).unwrap_or_default() {
        match child.name().value() {
            // Prose for a human, not consumed by anything. Kept in the
            // manifest so the reason for an option lives beside it.
            "description" => {}
    ...
    // Required, because an option with no default is a required argument
...
/// characters that would end the quoting or start an expansion are
/// rejected rather than escaped. A list joins on spaces, so a value
...
/// the selected variant, then the value in modules.kdl. Only the image
