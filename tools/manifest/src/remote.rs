...
//! resolve and nothing to record separately: the pin in modules.kdl is
//! the whole statement.
//!
//! The hash is mandatory rather than optional, because what a remote
//! module ships is arbitrary shell running as root in the build. A
//! changed ref whose archive does not hash to the pinned value fails
...
/// Where fetched module trees land, relative to `modules/`. Inside the
/// module tree rather than under `build/` with the other generated
/// output, because the ctx stage copies `modules/` and a `COPY` cannot be
/// made conditional on a directory that only exists once something is
/// pinned.
pub const REMOTE_DIR: &str = ".remote";

/// The archives the fetch can extract. One leading directory is always
/// stripped, which is a tar option and the reason a zip is rejected here
/// rather than half-extracted later.
const ARCHIVES: [&str; 5] = [".tar.gz", ".tgz", ".tar.xz", ".tar.zst", ".tar.bz2"];

pub struct Remote {
    /// Unexpanded, `{ref}` included, because this is what a reviewer
    /// reads and what the checksum workflow rewrites around.
    pub url: String,
    pub git_ref: String,
    pub sha256: String,
    /// The module's directory inside the archive, relative to its root
    /// once the leading directory is stripped. Absent means the archive
    /// root is the module, which is the single-module repository.
...
///     ref "module-name/v1.2.0"
///     sha256 "..."
///     path "modules/module-name"
/// }
/// ```
///
/// The same shape as an `asset` block, and for the same reasons: exactly
/// one of `renovate` and `manual`, and the tracked line directly below
...
pub fn parse(node: &KdlNode, file: &str, text: &str, issues: &mut Issues) -> Option<Remote> {
    ...
    for child in node.children().map(|c| c.nodes()).unwrap_or_default() {
        ...
        match kind {
            "renovate" => {
                ...
                // The two the manager in renovate.json5 matches for a
                // module pin. git-refs tracks a branch head, so it needs
                // the branch name as well as the commit, and a module is
                // published as a per-module tag rather than a branch.
    ...
    // Mandatory, unlike an asset's, and the reason a remote module is
    // safe to expand into the build at all: the manifest and the scripts
    // that run as root are exactly the bytes that were reviewed.
...
fn check_url(remote: &Remote, tracked: bool, file: &str, text: &str, issues: &mut Issues) {
    ...
    // A tracked pin whose URL ignores the ref would keep serving the same
    // archive after a bump, so Renovate would move the ref and the build
    // would not notice.
    ...
    // Both reach the fetch script through a pipe-separated line and then
    // a shell command, so the characters that would break either are
