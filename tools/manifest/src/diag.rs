...
//!
//! Every error points at the line in the manifest that caused it, because
//! the whole reason validation runs on the host is to be actionable in
//! seconds. "unsatisfied requirement" with no location is a grep away from
//! useless once there are forty modules.
...
/// Collects every problem before reporting, so one run surfaces all of
/// them. Stopping at the first turns a five minute fix into five
/// edit-run cycles.
#[derive(Default)]
